        ...
        methods: {
            submitForm(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        const _this = this;
                        axios.post("http://localhost:8181/book/updateBook", _this.ruleForm).then(function (response) {
                            if (response.data == "1") {
            ...
            axios.get("http://localhost:8181/book/findById/" + _this.$route.query.id,).then(function (response) {
